...
double TimeScoreHomework::getResult() const{
    ...
    if(!TimeHomework::getResult()){
        ...
        if(now.Year()>deadline.Year() || (now.Year()==deadline.Year() && now.Month()>deadline.Month())
                || (now.Year()==deadline.Year() && now.Month()==deadline.Month())){

            if((static_cast<int>(now.Day())-static_cast<int>(deadline.Day()))>=day_max_malus)
                somma-=3;

            else if((static_cast<int>(now.Day())-static_cast<int>(deadline.Day()))>=(day_max_malus-1))
                somma-=2;
